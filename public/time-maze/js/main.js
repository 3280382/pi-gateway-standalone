...
class ShadowClone {
  ...
  render(ctx) {
    ...
    ctx.beginPath();
    ctx.roundRect(this.x - this.radius, this.y - this.radius, this.radius * 2, this.radius * 2, 6);
    ctx.fill();
...
class Player extends Entity {
  ...
  tryMove(dx, dy, entities, width, height) {
    ...
    if (dx !== 0) {
      ...
      for (const entity of entities) {
        if (entity.type === "wall" || (entity.type === "door" && entity.isSolid())) {
  ...
  render(ctx) {
    ...
    ctx.fillStyle = "rgba(255, 255, 255, 0.3)";
    ctx.beginPath();
    ctx.roundRect(this.x - this.radius + 2, this.y - this.radius + 2, size - 4, size - 4, 4);
    ctx.fill();
...
class LevelManager {
  constructor() {
    ...
    this.levels = [this.createLevel1(), this.createLevel2(), this.createLevel3()];
  }
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        switch (char) {
          ...
          case "S": {
            const switchId = `S1`;
            entities.push(new Switch(x, y, switchId, config.switchTargets[switchId] || []));
            break;
          }
          case "D": {
            const doorId = `D1`;
            entities.push(new Door(x, y, this.tileSize, this.tileSize, doorId, false));
            break;
          }
          case "O":
...
class GameEngine {
  ...
  createShadow(rewindInfo) {
    ...
    if (shadowStates.length > 5) {
      const shadow = new ShadowClone(shadowStates, `shadow_${this.shadowCounter++}`);
      this.shadowClones.push(shadow);
      this.score += 50;

      document.getElementById("shadow-count").textContent = this.shadowClones.filter(
        (s) => s.active
      ).length;
  ...
  update() {
    ...
    if (this.timeManager.isRewinding) {
      ...
      if (state) {
        StateRecorder.applyState(state, this.player, this.levelManager.entities, this.shadowClones);
      ...
      this.player.update(this.input, entities, this.logicalWidth, this.logicalHeight, false);
      ...
      const { reachedExit } = this.levelManager.checkInteractions(this.player, this.shadowClones);
      if (reachedExit) {
        this.score += 1000;
  ...
  updateUI() {
    ...
    if (timelineCurrent && this.timeManager.timeline.length > 0) {
      const progress = (this.timeManager.currentIndex / this.timeManager.maxFrames) * 100;
