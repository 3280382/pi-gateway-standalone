...
(function () {
      ...
      try {
        ...
        if (!fiberKey) {
          ...
          for (let i = 0; i < 3 && parent; i++) {
            ...
            const parentFiberKey = parentKeys.find(
              (k) => k.startsWith("__reactFiber") || k.startsWith("__reactInternal")
            );
            if (parentFiberKey) {
              el = parent;
        ...
        while (current && depth < maxDepth) {
          ...
          if (current.type) {
            ...
            if (typeof current.type === "function") {
              info.name = current.type.displayName || current.type.name || "Anonymous";
  ...
  // ============================================
  // AIDescriptionGenerator - AI 描述生成器 (简洁版)
  // ============================================
  class AIDescriptionGenerator {
    ...
    generate(context) {
      ...
      if (context.react?.componentName) {
        parts.push(`<${context.react.componentName}>`);
      }
      ...
      // 3. 当前元素描述
      const elementDesc = this.describeElement(context);
    ...
    // 获取最多3级父元素链
    getParentChain(context) {
      const chain = [];
      const segments = context.path?.segments || [];

      // 跳过 html/body，取最近3级有意义的父元素
      for (let i = Math.max(0, segments.length - 4); i < segments.length - 1; i++) {
        const seg = segments[i];
        if (seg.tag === "html" || seg.tag === "body") continue;

        const name = this.getElementShortName(seg);
    ...
    describeElement(context) {
      ...
      if (dom.type && dom.type !== "text") {
        parts.push(`type="${dom.type}"`);
      }

      // 文本内容（简短的）
      if (dom.text && dom.text.length <= 30 && !dom.text.includes("\n")) {
        parts.push(`"${dom.text}"`);
      }
    ...
    getIdentifiers(context) {
      ...
      if (context.dom.id) {
        ids.push(`#${context.dom.id}`);
      }
      if (context.markers?.aiId) {
        ids.push(`data-ai-id="${context.markers.aiId}"`);
      }
      if (context.markers?.testId) {
        ids.push(`data-testid="${context.markers.testId}"`);
      }
      if (context.markers?.component) {
        ids.push(`data-component="${context.markers.component}"`);
      }
    ...
    getKeyClasses(context) {
      ...
      const classes = (context.dom.classList || [])
        .filter((c) => {
          // 过滤掉 ui-marker 相关
          if (c.startsWith("ui-marker")) return false;
          // 过滤掉通用工具类
          if (genericPrefixes.some((p) => c.startsWith(p))) return false;
          // 保留有意义的类名（通常包含组件名或语义）
          return c.length > 2;
        })
        .slice(0, 3); // 最多3个
    ...
    // 获取元素短名称
    getElementShortName(seg) {
      if (!seg) return "";

      // 优先使用 ID
      if (seg.id) {
        return `${seg.tag}#${seg.id}`;
      }
      ...
      if (seg.className) {
        const classes = seg.className
          ...
          .filter(
            (c) =>
              c.length > 3 &&
              !c.startsWith("ui-marker") &&
              !["container", "wrapper", "content", "main"].includes(c)
          );
        if (classes.length > 0) {
          return `${seg.tag}.${classes[0]}`;
        }
      }
  ...
  class UIRenderer {
    ...
    showDialog(marker, context, onSubmit, onCancel) {
      ...
      info.style.cssText =
        "background: #f3f4f6; padding: 12px; border-radius: 6px; margin-bottom: 16px; font-size: 13px; color: #666;";
      info.innerHTML = `
      ...
      shortcuts.innerHTML =
        '<div style="font-size: 12px; color: #999; margin-bottom: 8px;">快捷选项：</div>' +
        options
          .map(
            (opt) => `<button class="ui-marker-shortcut" style="
          margin-right: 8px;
          margin-bottom: 8px;
          padding: 4px 12px;
          border: 1px solid #ddd;
  ...
  class UIMarker {
    constructor(userConfig = {}) {
      ...
      this.aiDescGen = new AIDescriptionGenerator(this.config);
    ...
    async selectMarker(marker) {
      ...
      // 生成 AI 定位描述
      const aiDescription = this.aiDescGen.generate(context);
      ...
      try {
        ...
        // 显示成功提示
        this.showToast("✅ AI 定位描述已复制到剪贴板", "success");

        // 同时输出到控制台
        console.log("%c[UIMarker] AI 定位描述已复制", "color: #6366f1; font-weight: bold;");
        ...
        console.error("[UIMarker] 复制到剪贴板失败:", err);
        this.showToast("❌ 复制失败，请查看控制台", "error");

        // 复制失败时，仍然输出到控制台
        console.log(
          "%c[UIMarker] AI 定位描述 (复制失败，请手动复制):",
