...
(function() {
  ...
  function shouldEnableDebug() {
    ...
    if (CONFIG.localhostPatterns.some(pattern => hostname.includes(pattern))) {
  ...
  function createFallbackConsole() {
    ...
    header.style.cssText = `
      ...
      align-items: center;
    `;
    header.innerHTML = '<span>📱 Debug Console</span><button id="debug-close" style="background:none;border:none;color:#fff;cursor:pointer;">✕</button>';
  ...
  function init() {
    ...
    console.log('[DebugSystem] Enabling debug mode, reason:', debugCheck.reason);
    isDebugEnabled = true;
