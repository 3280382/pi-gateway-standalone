...
1. **工具菜单控制**: 点击右下角 🔧 工具图标 → 选择 **UI Marker** 开关
2. **浮动按钮**: 激活后右下角显示红色 **×** 按钮，点击即可停用
3. **紧急恢复**: 在浏览器控制台输入 `window.forceDisableUIMarker()`
5. **描述**: 输入修改需求，发送给 AI
...
### Q: 手机如何使用？
手机无法使用 `Ctrl+Shift+M` 快捷键，提供三种替代方式：

**方式一：工具菜单控制**
1. 点击页面右下角 🔧 工具图标
2. 在弹出的菜单中点击 **UI Marker** 开关
3. 显示 ✓ 表示已激活
...
如果无法通过上述方式停用：
1. 打开浏览器开发者工具（控制台）
2. 输入命令：`window.forceDisableUIMarker()`
3. 或清除浏览器 localStorage 后刷新页面

### Q: 点击元素没反应？
1. 确认 UI Marker 已激活 (检查右下角是否有红色 × 按钮)
2. 检查元素是否被其他元素遮挡
3. 尝试刷新页面
