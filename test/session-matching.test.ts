...
test.describe("Session Matching Mechanism", () => {
  ...
  test("3. 多客户端消息隔离", async () => {
    ...
    await Promise.all([waitForOpen(ws1, 10000), waitForOpen(ws2, 10000)]);
    ...
    ws1.send(
      JSON.stringify({
        ...
        workingDir: "/root/pi-gateway-standalone",
        sessionFile:
          "/root/.pi/agent/sessions/--root-pi-gateway-standalone--/2026-04-17T08-21-02-271Z_019d9a87-76bf-7650-8953-47c6f68a2a96.jsonl",
      })
    );
    ...
    ws2.send(
      JSON.stringify({
        ...
        workingDir: "/root/pi-gateway-standalone",
        sessionFile:
          "/root/.pi/agent/sessions/--root-pi-gateway-standalone--/2026-04-17T09-32-26-673Z_019d9ac8-d6b0-75fd-afd2-fc6a5833bbdf.jsonl",
      })
    );
    ...
    const _client2HasMessage = client2Messages.includes("message_start");
