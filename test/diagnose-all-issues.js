...
async function diagnoseAllIssues() {
  ...
  for (const check of cssChecks) {
    const regex = new RegExp(`${check.selector}\\s*{[^}]*${check.property}:\\s*([^;]+)`);
    ...
    if (value !== check.expected) {
      issues.push(`${check.selector}的${check.property}应为${check.expected}，实际为${value}`);
    }
  }
  ...
  for (const check of executeChecks) {
    ...
    if (exists) {
      ...
      if (check.file.includes("file.controller.ts")) {
        ...
        if (expectsCommand) {
          issues.push("后端executeCommand期望{command, cwd, streaming}，但前端发送{path}");
        }
      }
    }
  }
  ...
  if (issues.length === 0) {
    ...
    if (issues.some((i) => i.includes("executeCommand期望"))) {
      ...
      console.log('   后端期望: { command: "./file.sh", cwd: "/path", streaming: true }');
