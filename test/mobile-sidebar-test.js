...
async function testMobileSidebar() {
  ...
  for (const ua of MOBILE_USER_AGENTS) {
    try {
      ...
      if (response.ok) {
        ...
        // 检查关键元素
        const checks = {
          hasToggleButton: html.includes("Toggle sidebar") || html.includes("toggle-sidebar"),
          hasSidebarElement: html.includes("sidebar") || html.includes("file-sidebar"),
          hasFileBrowser: html.includes("FileBrowser") || html.includes("file-browser"),
        ...
        console.log(`     侧边栏元素: ${checks.hasSidebarElement ? "✅" : "❌"}`);
  ...
  try {
    ...
    const checks = {
      ...
      hasConditionalRender: fsCode.includes("if (!visible)") || fsCode.includes("visible &&"),
      hasMobileStyles: fsCode.includes("@media") || fsCode.includes("max-width"),
