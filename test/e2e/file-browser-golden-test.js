...
class FileBrowserGoldenTest {
  ...
  async testAPIEndpoints() {
    ...
    try {
      ...
      this.log("GET /api/version", res.ok && data.version, `版本: ${data.version}`);
    } catch (e) {
    ...
    try {
      ...
      const data = await res.json();
      this.log("POST /api/browse (/)", res.ok, `${data.items?.length || 0} 个项目`);
    } catch (e) {
    ...
    // 5. 读取文件API
    try {
      const res = await fetch(`${API_URL}/api/files/content?path=/root/README.md`);
      this.log("GET /api/files/content", [200, 404].includes(res.status), `HTTP ${res.status}`);
    } catch (e) {
    ...
    const checks = [
      {
        ...
        patterns: ["export function FileBrowser", "FileSidebar", "FileToolbar", "FileViewer"],
      },
      {
        name: "FileSidebar侧边栏",
        file: "src/client/components/files/FileSidebar.tsx",
        patterns: ["export function FileSidebar", "visible", "loadDirectory", "TreeCache"],
      },
      {
        name: "FileViewer查看器",
        file: "src/client/components/files/FileViewer.tsx",
        patterns: ["export function FileViewer", "isOpen", "mode", "readFile", "writeFile"],
      },
      {
        name: "FileList文件列表",
      ...
      },
      {
        ...
        patterns: ["export function FileActionBar", "executeFile", "selectedActionFile"],
      },
      {
        name: "FileToolbar工具栏",
      ...
      },
      {
        ...
        patterns: ["browseDirectory", "readFile", "writeFile", "executeFile", "formatFileSize"],
      },
      {
        name: "fileStore状态管理",
      ...
      },
      {
        ...
        patterns: ["useFileViewerStore", "isOpen", "openViewer", "closeViewer", "mode"],
      },
    ];
  ...
  async testFeatureImplementation() {
    ...
    const features = [
      ...
      {
        name: "文件排序",
        check: () => content.includes("sortMode") && content.includes("getFilteredAndSortedItems"),
      },
      {
        name: "文件过滤",
        check: () => content.includes("filterText") || content.includes("filterType"),
      },
      {
        name: "文件预览",
        check: () =>
          fs.existsSync("/root/pi-gateway-standalone/src/client/stores/fileViewerStore.ts"),
      },
      {
        name: "文件编辑",
        check: () => {
          ...
          return viewerContent.includes("edit") && viewerContent.includes("save");
        },
      },
      { name: "文件执行", check: () => content.includes("executeFile") },
      {
        name: "批量选择",
        check: () => content.includes("selectedItems") || content.includes("toggleSelection"),
      },
      {
        name: "底部面板集成",
        check: () => content.includes("onOpenBottomPanel") || content.includes("onExecuteOutput"),
      },
    ];
  ...
  async testFrontendAccess() {
    ...
    try {
      ...
      this.log("首页访问", res.ok, `HTTP ${res.status}, ${(await res.text()).length} bytes`);
    } catch (e) {
    ...
    for (const file of files) {
      try {
        ...
        this.log(`组件访问: ${path.basename(file)}`, res.ok, `HTTP ${res.status}`);
      } catch (e) {
  ...
  async testDataStructure() {
    ...
    try {
      ...
      const checks = [
        ...
        {
          name: "items项有name",
          test: () => data.items.length === 0 || typeof data.items[0].name === "string",
        },
        {
          name: "items项有path",
          test: () => data.items.length === 0 || typeof data.items[0].path === "string",
        },
        {
          name: "items项有isDirectory",
          test: () => data.items.length === 0 || typeof data.items[0].isDirectory === "boolean",
        },
      ];
