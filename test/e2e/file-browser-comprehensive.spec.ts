...
test.describe("文件浏览器功能验证", () => {
	...
	test("文件浏览器主界面渲染", async ({ page }) => {
		...
		const fileList = page
			.locator(
				'[class*="fileList"], [class*="fileGrid"], [class*="contentArea"]',
			)
			.first();
	...
	test("视图切换按钮工作", async ({ page }) => {
		...
		const viewButtons = page
			.locator(
				'button:has-text("Grid"), button:has-text("List"), [class*="viewMode"]',
			)
			.first();
		...
		if (await viewButtons.isVisible().catch(() => false)) {
			...
			// 验证切换后界面仍然正常
			const fileList = page
				.locator('[class*="fileList"], [class*="fileGrid"]')
				.first();
			await expect(fileList).toBeVisible();
		}
	});
	...
	test("底部菜单按钮工作", async ({ page }) => {
		...
		const menuButtons = page.locator(
			'nav button, [class*="bottomMenu"] button',
		);
	...
	test("文件浏览器响应式布局", async ({ page }) => {
		...
		for (const viewport of viewports) {
			...
			const fileBrowser = page
				.locator('[class*="fileBrowser"], section')
				.first();
