...
test.describe("文件浏览器 - 核心功能验证", () => {
	...
	test("1. 文件浏览器主界面渲染", async ({ page }) => {
		...
		const contentArea = page
			.locator('[class*="contentArea"], [class*="main"]')
			.first();
	...
	test("2. 侧边栏目录树功能", async ({ page }) => {
		...
		const sidebarButton = page
			.locator('nav button, [class*="bottomMenu"] button')
			.first();
	...
	test("5. 文件过滤功能", async ({ page }) => {
		...
		const filterInput = page
			.locator(
				'input[type="text"], input[placeholder*="search" i], input[placeholder*="filter" i]',
			)
			.first();
	...
	test("6. 文件选择和预览", async ({ page }) => {
		...
		// 查找可点击的文件项
		const fileItems = page
			.locator('[class*="fileItem"], [class*="gridItem"]')
			.first();
	...
	test("7. 文件执行和底部面板", async ({ page }) => {
		...
		const executeButton = page
			.locator(
				'button:has-text("Execute"), button:has-text("Run"), [class*="execute"]',
			)
			.first();
		...
		if (await executeButton.isVisible().catch(() => false)) {
			...
			const bottomPanel = page
				.locator(
					'[class*="bottomPanel"], [class*="terminal"], [class*="output"]',
				)
				.first();
