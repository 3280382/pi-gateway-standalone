...
class FileBrowserTest {
  ...
  async testAPIEndpoints() {
    ...
    try {
      ...
      await this.log("POST /api/execute", response.ok, `状态: ${response.status}`);
    } catch (error) {
  ...
  async testCodeIntegrity() {
    ...
    const checks = [
      ...
      },
      {
        ...
        checks: ["export function FileSidebar", "visible: boolean", "loadDirectory"],
      },
      {
        name: "FileViewer组件存在",
      ...
      },
      {
        ...
        checks: ["useFileStore", "currentPath", "setItems", "getFilteredAndSortedItems"],
      },
    ];
    ...
    for (const check of checks) {
      ...
      const missing = check.checks.filter((pattern) => !content.includes(pattern));
  ...
  async testStateFlow() {
    ...
    await this.log("初始路径为/root", hasRootPath, hasRootPath ? "配置正确" : "配置错误");

    // 检查关键方法
    const methods = ["setItems", "setCurrentPath", "getFilteredAndSortedItems", "selectForAction"];
  ...
  async testFrontendAccessibility() {
    ...
    for (const file of files) {
      try {
        ...
        await this.log(`文件访问 ${path.basename(file)}`, response.ok, `HTTP ${response.status}`);
      } catch (error) {
