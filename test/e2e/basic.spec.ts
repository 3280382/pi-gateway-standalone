...
test.describe("文件浏览器基本功能测试", () => {
	test("应用应该正常加载", async ({ page }) => {
		...
		const errorElements = page
			.locator("body")
			.filter({ hasText: /error|Error|失败|崩溃/i });
	...
	test("应该能够切换到文件浏览器标签页", async ({ page }) => {
		...
		if (await filesTab.isVisible()) {
			...
			// 验证文件浏览器相关元素出现
			const fileBrowserElements = page
				.locator("div")
				.filter({ hasText: /file|文件|browse|浏览/i });
	...
	test("API端点应该正常工作", async ({ page }) => {
		...
		const hasSettingsApi = apiRequests.some((req) =>
			req.includes("/api/settings"),
		);
