...
async function testOverlaySidebar() {
  ...
  try {
    ...
    const viewButtons = Array.from(document.querySelectorAll("button")).filter(
      (btn) =>
        btn.textContent && (btn.textContent.includes("Chat") || btn.textContent.includes("Files"))
    );
    ...
    if (viewButtons.length >= 2) {
      ...
      // 检查当前激活的视图
      const activeViewButton = viewButtons.find((btn) => btn.className.includes("active"));
      if (activeViewButton) {
  ...
  try {
    ...
    const styleChecks = {
      hasFixedPosition: cssContent.includes("position: fixed") && cssContent.includes(".sidebar"),
      hasTransform: cssContent.includes("transform: translateX") && cssContent.includes(".sidebar"),
      hasZIndex: cssContent.includes("z-index") && cssContent.includes(".sidebar"),
      hasVisibleClass: cssContent.includes(".sidebar.visible"),
      hasTransition: cssContent.includes("transition") && cssContent.includes(".sidebar"),
    };

    console.log(`   position: fixed: ${styleChecks.hasFixedPosition ? "✅" : "❌"}`);
    console.log(`   transform动画: ${styleChecks.hasTransform ? "✅" : "❌"}`);
    console.log(`   z-index: ${styleChecks.hasZIndex ? "✅" : "❌"}`);
    console.log(`   .sidebar.visible类: ${styleChecks.hasVisibleClass ? "✅" : "❌"}`);
    console.log(`   transition过渡: ${styleChecks.hasTransition ? "✅" : "❌"}`);
