      ...
      switch (step.action) {
        case "browse": {
          ...
          if (browseResult.success) {
            ...
            if (browseResult.data.items.length > 3) {
              console.log(`     ... 还有 ${browseResult.data.items.length - 3} 个项目`);
            }
        ...
        case "read": {
          ...
          if (readResult.success) {
            console.log(`  ✅ 读取成功: ${readResult.data.content.length} 字符`);
            console.log(
        ...
        case "find": {
          ...
          if (findResult.success) {
            ...
            console.log(`  ✅ 查找完成: 找到 ${matchingFiles.length} 个匹配文件`);
        ...
        case "read_sample": {
          ...
          if (sampleResult.success) {
            const files = sampleResult.data.items.filter((item) => !item.isDirectory);
            const samples = files.slice(0, step.count || 2);

            for (const file of samples) {
              const fileResult = await readFile(file.path);
              if (fileResult.success) {
                console.log(`  ✅ 读取 ${file.name}: ${fileResult.data.content.length} 字符`);
              }
            }
            successCount++;
          }
        ...
        case "check_executable": {
          ...
          if (checkResult.success) {
            ...
            console.log(`  ✅ 检查完成: 找到 ${executables.length} 个可执行文件`);
