...
test.beforeEach(async ({ page }) => {
  ...
  page.on("requestfailed", (request) => {
    browserHelper.logConsole("error", `请求失败: ${request.url()} - ${request.failure()?.errorText}`);
  });
...
test.describe("Terminal Panel 浏览器测试", () => {
  ...
  test("创建新终端会话", async ({ page }) => {
    ...
    const newSessionBtn = page.locator("button[title*='New terminal' i], button[class*='addTabBtn' i], button:has-text('Create Terminal')").first();
  ...
  test("多个终端标签页", async ({ page }) => {
    ...
    for (let i = 0; i < 3; i++) {
      const newSessionBtn = page.locator("button[title*='New terminal' i], button[class*='addTabBtn' i]").first();
  ...
  test("终端全屏切换", async ({ page }) => {
    ...
    const fullscreenBtn = page.locator("button[title*='Fullscreen' i], button[title*='fullscreen' i]").first();
