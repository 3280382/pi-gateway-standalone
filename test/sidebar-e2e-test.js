...
async function testSidebarCompleteWorkflow() {
  ...
  try {
    ...
    const toolbarCoverage = {
      ...
      hasButtonRenderingTest: toolbarTestCode.includes("getByTitle('Toggle sidebar')"),
    };

    console.log(`   ✅ 有切换按钮测试: ${toolbarCoverage.hasToggleButtonTest}`);
    console.log(`   ✅ 有按钮点击测试: ${toolbarCoverage.hasToggleClickTest}`);
    console.log(`   ✅ 有按钮渲染测试: ${toolbarCoverage.hasButtonRenderingTest}`);
  ...
  try {
    ...
    const sidebarCoverage = {
      hasVisiblePropTest: sidebarTestCode.includes("visible") && sidebarTestCode.includes("prop"),
      hasConditionalRenderTest:
        sidebarTestCode.includes("if (!visible)") || sidebarTestCode.includes("visible &&"),
      hasTreeContentTest: sidebarTestCode.includes("tree") || sidebarTestCode.includes("directory"),
      hasAPILoadingTest:
        sidebarTestCode.includes("browseDirectory") || sidebarTestCode.includes("API"),
    };

    console.log(`   ✅ 有visible属性测试: ${sidebarCoverage.hasVisiblePropTest}`);
    console.log(`   ✅ 有条件渲染测试: ${sidebarCoverage.hasConditionalRenderTest}`);
  ...
  try {
    ...
    console.log(`   FileSidebar有visible条件检查: ${hasVisibleCheck ? "✅" : "❌"}`);
    console.log(`   FileSidebar有条件CSS类: ${hasConditionalClass ? "✅" : "❌"}`);
  ...
  try {
    ...
    const hasToggleButton =
      toolbarCode.includes("Toggle sidebar") || toolbarCode.includes("toggle-sidebar");

    const hasOnToggleSidebar = toolbarCode.includes("onToggleSidebar");

    console.log(`   FileToolbar有切换按钮: ${hasToggleButton ? "✅" : "❌"}`);
    console.log(`   FileToolbar有onToggleSidebar回调: ${hasOnToggleSidebar ? "✅" : "❌"}`);
