...
async function verifySidebarFix() {
  ...
  try {
    ...
    console.log(`1. FileToolbar有侧边栏按钮: ${hasSidebarButton ? "✅" : "❌"}`);

    if (hasSidebarButton) {
      console.log("   按钮代码已添加:");
      const buttonLines = toolbarCode
        .split("\n")
        .filter((line) => line.includes("Toggle sidebar") || line.includes("Sidebar</span>"))
        .slice(0, 3);
    ...
    const passesToggleSidebar = browserCode.includes("onToggleSidebar={onToggleSidebar}");
    ...
    console.log(`3. App.tsx传递onToggleSidebar给FileBrowser: ${appPassesToggle ? "✅" : "❌"}`);
    ...
    if (hasSidebarButton && passesToggleSidebar && appPassesToggle) {
      ...
      if (!passesToggleSidebar) console.log("   - FileBrowser未正确传递onToggleSidebar");
      if (!appPassesToggle) console.log("   - App.tsx未传递onToggleSidebar给FileBrowser");
    }
  } catch (error) {
