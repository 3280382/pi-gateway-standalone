...
async function diagnoseSidebarIssue() {
	...
	try {
		...
		const hasFileSidebar =
			html.includes("FileSidebar") ||
			html.includes("file-sidebar") ||
			html.includes("sidebar");

		console.log(
			`   HTML中包含FileSidebar相关代码: ${hasFileSidebar ? "✅" : "❌"}`,
		);
	...
	try {
		...
		const importsFileSidebar =
			appCode.includes("FileSidebar") ||
			appCode.includes("./components/files/FileSidebar");

		console.log(
			`   App.tsx导入FileSidebar: ${importsFileSidebar ? "✅" : "❌"}`,
		);
		...
		const fbImportsSidebar =
			fbCode.includes("FileSidebar") || fbCode.includes("./FileSidebar");

		console.log(
			`   FileBrowser.tsx导入FileSidebar: ${fbImportsSidebar ? "✅" : "❌"}`,
		);
	...
	try {
		...
		for (const test of apiTests) {
			...
			if (response.ok) {
				...
				if (!hasDirectories) {
					issues.push(
						`${test.name}返回的数据中没有目录项，侧边栏无法显示树结构`,
					);
				}
			} else {
	...
	try {
		...
		const foundStyles = sidebarStyles.filter((style) =>
		...
		const hasHidingStyles = hidingStyles.some((style) =>
			cssContent.includes(style),
		);
		console.log(
			`   有隐藏样式: ${hasHidingStyles ? "⚠️ 可能有隐藏样式" : "✅ 无"}`,
		);
	} catch (error) {
		console.log(`   ❌ 错误: ${error.message}`);
	}
