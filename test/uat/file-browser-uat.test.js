	...
	async run() {
		...
		await this.test("FileSidebar加载逻辑正确", async () => {
			...
			const hasLoadRootCall =
				/useEffect\(\(\)\s*=>\s*\{[\s\S]*?loadRoot\(\)[\s\S]*?\},\s*\[\s*visible/.test(
					content,
				);
		...
		await this.test("文件列表容器高度设置正确", async () => {
			...
			const hasFlex =
				content.includes("flex: 1") || content.includes("flex-grow");
			const hasHeight =
				content.includes("height: 100%") || content.includes("height: 100vh");
		...
		await this.test("FileList有选择处理逻辑", async () => {
			...
			const hasSelectLogic =
				content.includes("selectForAction") || content.includes("onClick");
			const hasDoubleClick =
				content.includes("onDoubleClick") || content.includes("dblclick");
		...
		await this.test("fileStore有选择相关方法", async () => {
			...
			return {
				...
				error: !(hasSelectForAction && hasSelectedActionFile)
					? "store缺少选择方法"
					: null,
			};
		});
		...
		await this.test("API端点全部可用", async () => {
			...
			return {
				...
				error:
					passed < endpoints.length
						? `${endpoints.length - passed} 个端点失败`
						: null,
			};
		});
