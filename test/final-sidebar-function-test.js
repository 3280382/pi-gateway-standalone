...
async function testSidebarFunctionWithLogging() {
	...
	const backendLogBefore = fs.readFileSync(
		path.join(logDir, "backend_current.log"),
		"utf8",
	);
	const backendLinesBefore = backendLogBefore
		.split("\n")
	...
	const backendLogAfter = fs.readFileSync(
		path.join(logDir, "backend_current.log"),
		"utf8",
	);
	const backendLinesAfter = backendLogAfter
		.split("\n")
		.filter((line) => line.trim());
	...
	for (const check of componentChecks) {
		try {
			...
			if (response.ok) {
				...
				const passedChecks = check.checks.filter((pattern) =>
					content.includes(pattern),
				);
				console.log(
					`   ${check.name}: ${passedChecks.length}/${check.checks.length} ✅`,
				);

				if (passedChecks.length < check.checks.length) {
					const failed = check.checks.filter(
