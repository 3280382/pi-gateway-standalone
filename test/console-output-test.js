...
async function testConsoleAndLogOutput() {
	...
	for (const logFile of logFiles) {
		...
		if (exists) {
			...
			console.log(
				`     大小: ${sizeKB} KB, 修改时间: ${stats.mtime.toLocaleString()}`,
			);
	...
	for (const test of apiTests) {
		try {
			...
			console.log(
				`   ${test.name}: ${response.status === 200 ? "✅" : "❌"} (${duration}ms)`,
			);
	...
	try {
		...
		console.log(
			`   前端页面: ${response.status === 200 ? "✅" : "❌"} (HTTP ${response.status})`,
		);
	...
	const backendLog = fs.readFileSync(
		path.join(logDir, "backend_current.log"),
		"utf8",
	);
	const frontendLog = fs.readFileSync(
		path.join(logDir, "frontend_current.log"),
		"utf8",
	);

	const backendHasColoredOutput =
		backendLog.includes("\x1b[") || backendLog.includes("[");
	const backendHasTimestamps =
		backendLog.includes("[") && backendLog.includes("]");
	const backendHasHTTPLogs =
		backendLog.includes("GET ") || backendLog.includes("POST ");

	const frontendHasViteOutput =
		frontendLog.includes("VITE") || frontendLog.includes("ready in");
	const frontendHasLocalURL =
		frontendLog.includes("Local:") || frontendLog.includes("127.0.0.1");
	...
	try {
		...
		const frontendRunning =
			statusOutput.includes("前端服务:") && statusOutput.includes("健康: ✅");
		const backendRunning =
			statusOutput.includes("后端服务:") && statusOutput.includes("健康: ✅");
	...
	console.log("\n或者查看日志文件:");
	console.log(
		"   tail -f /root/pi-gateway-standalone/logs/frontend_current.log",
	);
	console.log(
		"   tail -f /root/pi-gateway-standalone/logs/backend_current.log",
	);
