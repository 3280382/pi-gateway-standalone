...
`;
	...
	try {
		...
		console.log(
			`   执行端点: ${response.ok ? "✅" : "❌"} (HTTP ${response.status})`,
		);
	...
	for (const check of codeChecks) {
		...
		const status =
			passed === check.checks.length ? "✅" : passed > 0 ? "⚠️" : "❌";
	...
	for (const check of cssChecks) {
		const pattern = new RegExp(
			`${check.selector}[^{]*{[^}]*${check.property}:\\s*([^;]+)`,
		);
		...
		if (match) {
			...
			console.log(
				`   ${check.selector} ${check.property}: ${correct ? "✅" : "❌"} ${value}`,
			);
	...
	const storeFile =
		"/root/pi-gateway-standalone/src/client/stores/fileStore.ts";
	...
	if (issues.length === 0) {
		...
		console.log(
			"   • 侧边栏设计: overlay面板 (position: fixed, transform动画)",
		);
		console.log(
			"   • 状态同步: App传递externalSidebarVisible和onToggleSidebar",
		);
