...
async function simulateBrowserTest() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const hasScripts = document.querySelectorAll('script[type="module"]').length > 0;
      const hasStyles = document.querySelectorAll('link[rel="stylesheet"]').length > 0;
  ...
  try {
    ...
    if (browseResponse.ok) {
      ...
      const validItems = browseData.items.filter(
        (item) => item.name && item.path && typeof item.isDirectory === "boolean"
      );

      if (validItems.length === browseData.items.length) {
        console.log(`     ✅ 所有项目数据结构正确`);
      } else {
        console.log(`     ⚠️ ${browseData.items.length - validItems.length} 个项目数据结构有问题`);
      }
  ...
  console.log(`状态: ${results.overall.success ? "✅ 所有测试通过" : "❌ 有测试失败"}`);
