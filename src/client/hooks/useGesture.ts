...
export function useGesture(options: UseGestureOptions = {}) {
  ...
  const onTouchMove = useCallback((e: React.TouchEvent) => {
    ...
    if (state.isPinching && touches.length === 2) {
      ...
      const scale = getPinchScale(pinchStartDistanceRef.current, currentDistance);
      onPinch?.(scale);
      return;
    }
  ...
  const onTouchEnd = useCallback((e: React.TouchEvent) => {
    ...
    startInfoRef.current = null;
  }, [disabled, state.isPinching, tapThreshold, doubleTapDelay, swipeThreshold, onTap, onDoubleTap, onSwipe, onPinchEnd, onPressEnd]);
