...
// Controllers removed - using services directly
...
export function useAppInitialization(): InitResult {
		...
		async (persistedDir: string | null, persistedSessionId: string | null) => {
			try {
				...
				if (wsConnected && persistedDir) {
					initializeSession(persistedDir, persistedSessionId, 5000).catch(
						(err) => {
	...
	const initializeSession = async (
		...
		try {
			console.log(
				`[AppInit] Calling initWorkingDirectory with timeout ${timeoutMs}ms`,
			);
			...
			const initData = {
				...
				sessionId:
					message?.sessionId || message?.session?.id || persistedSessionId,
			...
			if (!initData.workingDir || !initData.sessionId) {
				console.warn(
					"[AppInit] initWorkingDirectory returned incomplete data:",
					message,
				);
				return;
			}
	...
	const syncSessionState = async (initData: {
		...
		try {
			...
			if (
				initData.workingDir &&
				sessionStore.currentDir !== initData.workingDir
			) {
			...
			if (
				initData.workingDir &&
				sidebarStore.workingDir?.path !== initData.workingDir
			) {
