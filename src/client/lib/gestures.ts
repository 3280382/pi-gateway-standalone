...
export function isSwipe(
  ...
  thresholds = { distance: GESTURE_CONFIG.SWIPE_THRESHOLD, velocity: GESTURE_CONFIG.SWIPE_VELOCITY }
): boolean {
...
export function analyzeGesture(
  ...
  if (distance < GESTURE_CONFIG.TAP_THRESHOLD && duration < GESTURE_CONFIG.LONG_PRESS_DELAY) {
    type = 'tap';
  } else if (duration >= GESTURE_CONFIG.LONG_PRESS_DELAY && distance < GESTURE_CONFIG.TAP_THRESHOLD) {
    type = 'longpress';
  } else if (distance >= GESTURE_CONFIG.SWIPE_THRESHOLD || velocity >= GESTURE_CONFIG.SWIPE_VELOCITY) {
    type = 'swipe';
  }
