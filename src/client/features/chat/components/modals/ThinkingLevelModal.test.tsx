...
describe("ThinkingLevelModal", () => {
  ...
  it("renders nothing when closed", () => {
    const { container } = render(<ThinkingLevelModal {...defaultProps} isOpen={false} />);
  ...
  it("shows description for each level", () => {
    ...
    expect(screen.getByTestId("level-low")).toHaveTextContent("Minimal thinking");
  ...
  it("marks current level as selected", () => {
    ...
    expect(screen.getByTestId("level-high")).toContainElement(screen.getByTestId("selected"));
  });

  it("calls onSelectLevel when level clicked", () => {
    const onSelectLevel = vi.fn();
    render(<ThinkingLevelModal {...defaultProps} onSelectLevel={onSelectLevel} />);
  ...
  it("handles all thinking level values", () => {
    ...
    levels.forEach((level) => {
      const { rerender } = render(<ThinkingLevelModal {...defaultProps} currentLevel={level} />);
      expect(screen.getByTestId(`level-${level}`)).toContainElement(screen.getByTestId("selected"));
      rerender(<></>);
    });
  });
});
