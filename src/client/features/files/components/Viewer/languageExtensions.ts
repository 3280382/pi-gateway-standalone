...
const javaLanguage = StreamLanguage.define({
  ...
  token: (stream, state: { inString: boolean }) => {
    ...
    if (stream.match(/\b(class|public|private|protected|static|void|return|if|else|for|while|import|package|new|this|extends|implements|interface|try|catch|finally|throw|throws|true|false|null)\b/)) {
      return "keyword";
    }
    
    if (stream.match(/\b(String|int|boolean|float|double|char|byte|short|long|Object|Class|void)\b/)) {
      return "typeName";
    }
...
const cppLanguage = StreamLanguage.define({
  ...
  token: (stream, state: { inString: boolean }) => {
    ...
    if (stream.match(/\b(int|char|float|double|void|if|else|for|while|return|class|struct|namespace|using|include|define|typedef|const|static|public|private|protected|virtual|new|delete|true|false|null|nullptr)\b/)) {
      return "keyword";
    }
...
const goLanguage = StreamLanguage.define({
  ...
  token: (stream, state: { inString: boolean }) => {
    ...
    if (stream.match(/\b(package|import|func|var|const|type|struct|interface|map|chan|if|else|for|range|return|switch|case|default|go|defer|panic|recover|nil|true|false)\b/)) {
      return "keyword";
    }
