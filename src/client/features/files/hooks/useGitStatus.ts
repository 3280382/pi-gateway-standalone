...
export function useGitStatus() {
	const { isGitModeActive, workingDir, items, updateFileGitStatuses } =
		useFileStore();
						...
						for (const [statusPath, status] of Object.entries(statuses)) {
							...
							if (statusPath.endsWith(relativePath)) {
								...
								if (pathLength === relLength) {
									...
									console.log(
										`跳过部分匹配: "${statusPath}" 以 "${relativePath}" 结尾但不是完整路径组件`,
									);
								}
							}
						}
