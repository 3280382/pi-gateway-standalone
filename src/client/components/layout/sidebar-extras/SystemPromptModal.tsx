...
export function SystemPromptModal() {
	...
	const activeTab = useSidebarExtrasStore(
		(state) => state.activeSystemPromptTab,
	);
	const prompts = useSidebarExtrasStore((state) => state.systemPrompts);
	const closeSystemPrompt = useSidebarExtrasStore(
		(state) => state.closeSystemPrompt,
	);
	const setActiveTab = useSidebarExtrasStore(
		(state) => state.setActiveSystemPromptTab,
	);
	const setSystemPrompts = useSidebarExtrasStore(
		(state) => state.setSystemPrompts,
	);
	const updateContent = useSidebarExtrasStore(
		(state) => state.updateSystemPromptContent,
	);
	...
	useEffect(() => {
		const loadPrompts = async () => {
			try {
				...
				const agentsContent = agentsResponse.ok
					? await agentsResponse.text()
					: "Failed to load AGENTS.md";

				// Fetch SYSTEM.md
				const systemResponse = await fetch("/api/system/system");
				const systemContent = systemResponse.ok
					? await systemResponse.text()
					: "Failed to load SYSTEM.md";
