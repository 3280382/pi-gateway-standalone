...
import { useSidebarExtrasStore } from '../../../../store/sidebarExtrasStore';
import { useSidebarController } from '../../../../api/sidebarApi';
import type { Theme, FontSize } from '../../../../types/sidebar';
...
export function Settings() {
  ...
  const setLlmLogConfig = useSidebarExtrasStore((state) => state.setLlmLogConfig);
      ...
      {llmLogConfig.enabled && (
        <div className={styles.setting}>
          ...
          <select
            ...
            onChange={(e) => setLlmLogConfig({ refreshInterval: Number(e.target.value) })}
          >
...
function FontSizeIcon({ size }: { size: FontSize }) {
  ...
  return (
    <span style={{ fontSize: sizes[size], fontWeight: 'bold' }}>A</span>
  );
}
