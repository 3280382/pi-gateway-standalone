			...
			>
				<div className={styles.header}>
					...
					<div className={styles.headerActions}>
						...
						<button className={styles.closeBtn} onClick={closeLlmLog}>
							✕
						</button>
					</div>
				</div>
				...
				<div className={styles.settings}>
					...
					<label>
						...
						<input
							...
							onChange={(e) =>
								setRefreshInterval(parseInt(e.target.value) || 5)
							}
							disabled={!enabled}
						/>
					</label>
					...
					{truncate && (
						<label>
							...
							<input
								...
								value={truncateSize}
								onChange={(e) =>
									setTruncateSize(parseInt(e.target.value) || 5000)
								}
							/>
						</label>
					)}
									...
									<div className={styles.logDetails}>
										{log.requestBody && (
											<div>
												...
												<pre className={styles.code}>
													{truncate
														? log.requestBody.slice(0, truncateSize)
														: log.requestBody}
												</pre>
											</div>
										)}
										{log.responseBody && (
											<div>
												...
												<pre className={styles.code}>
													{truncate
														? log.responseBody.slice(0, truncateSize)
														: log.responseBody}
												</pre>
											</div>
										)}
