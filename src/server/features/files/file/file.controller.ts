...
export async function browse(req: Request, res: Response) {
  ...
  try {
    ...
    logger.error(`文件浏览错误: ${error instanceof Error ? error.message : String(error)}`, {
      targetPath,
    });
    res.status(500).json({
      error: String(error),
...
export async function content(req: Request, res: Response) {
  ...
  try {
    ...
    logger.error(`获取文件内容错误: ${error instanceof Error ? error.message : String(error)}`, {
      targetPath,
    });
    res.status(500).json({ error: String(error) });
  }
...
export async function raw(req: Request, res: Response) {
  ...
  try {
    ...
    logger.error(`获取原始文件错误: ${error instanceof Error ? error.message : String(error)}`, {
      targetPath,
    });
    res.status(500).json({ error: String(error) });
  }
...
export async function write(req: Request, res: Response) {
  ...
  try {
    ...
    logger.error(`写入文件错误: ${error instanceof Error ? error.message : String(error)}`, {
      targetPath,
    });
    res.status(500).json({ error: String(error) });
  }
...
export async function batchDelete(req: Request, res: Response) {
  ...
  try {
    ...
    // 第二阶段：执行删除
    logger.info(`开始批量删除: ${validatedPaths.length} 个项目, 总大小: ${totalSize} 字节`);
