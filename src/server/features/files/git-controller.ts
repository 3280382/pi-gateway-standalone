...
async function getGitHistory(
	...
	console.log(
		`[GitController] History: gitRoot=${gitRoot}, relativePath=${relativePath}`,
	);
...
async function getFileDiff(
	...
	try {
		...
		console.error(
			"[GitController] Error getting diff:",
			error?.stderr || error?.message || error,
		);
		throw new Error(
			`Failed to get diff: ${error?.stderr || error?.message || "Unknown error"}`,
		);
	}
}
...
export async function getGitHistoryHandler(
	...
	console.log(
		`[GitController] History request: filePath=${filePath}, workingDir=${workingDir}`,
	);
