...
export async function handleChangeDir(
	...
	logger.info(
		`[change_dir]    - ctx.session.session exists: ${!!ctx.session?.session}`,
	);
	logger.info(
		`[change_dir]    - ctx.session.workingDir: "${ctx.session?.workingDir}"`,
	);
	...
	try {
		...
		logger.info(
			`[change_dir] 4. Path existence check: existsSync("${newPath}") = ${pathExists}`,
		);
		...
		logger.info(
			`[change_dir] 5. Path exists, calling ctx.session.initialize("${newPath}")...`,
		);
		...
		logger.info(
			`[change_dir] 7. Sending response to client: ${JSON.stringify(response)}`,
		);
		...
		logger.error(
			`[change_dir] Stack: ${error instanceof Error ? error.stack : "N/A"}`,
		);
