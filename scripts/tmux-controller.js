...
const controller = {
	...
	init() {
		...
		if (!sessionExists()) {
			log(
				"⚠️ Tmux 会话不存在，请先运行: bash scripts/tmux-dev.sh create",
				"yellow",
			);
			return false;
		}
	...
	async startFrontend() {
		...
		try {
			if (existsSync(frontendLog)) {
				...
				const oldTimestamp = new Date(stats.mtime)
					.toISOString()
					.replace(/[:.]/g, "-")
					.slice(0, 19);
		...
		if (health) {
			...
			log(
				`⚠️ 前端可能未成功启动，请检查窗格 0 或日志: frontend_current.log`,
				"yellow",
			);
		}
		return health;
	...
	async startBackend() {
		...
		if (health) {
			...
			log(
				`⚠️ 后端可能未成功启动，请检查窗格 1 或日志: backend_current.log`,
				"yellow",
			);
		}
		return health;
	...
	clearCache() {
		...
		runInPane(
			2,
			`cd ${GATEWAY_DIR} && rm -rf node_modules/.vite && echo 'Vite 缓存已清除'`,
		);
		log("✅ 缓存清除命令已发送", "green");
	},
