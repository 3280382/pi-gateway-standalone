 ...
 *
 * 使用方式:
 * import controller from './tmux-controller.js';
...
const controller = {
  ...
  async startFrontend() {
    log("🚀 启动前端服务...", "blue");
    const frontendLog = `${GATEWAY_DIR}/logs/frontend_current.log`;
    // 重命名旧日志文件
    try {
      if (existsSync(frontendLog)) {
        const stats = statSync(frontendLog);
        const oldTimestamp = new Date(stats.mtime).toISOString().replace(/[:.]/g, "-").slice(0, 19);
    ...
    runInPane(
      0,
      `mkdir -p ${GATEWAY_DIR}/logs && cd ${GATEWAY_DIR} && echo '前端日志: ${frontendLog}' && npx vite --host 127.0.0.1 --port 5173 2>&1 | tee -a '${frontendLog}'`
    );
    await sleep(3000);
    ...
    if (health) {
      log(`✅ 前端启动成功 (当前日志: frontend_current.log)`, "green");
    } else {
      log(`⚠️ 前端可能未成功启动，请检查窗格 0 或日志: frontend_current.log`, "yellow");
    }
    return health;
  },
  ...
  async startBackend() {
    log("🚀 启动后端服务...", "blue");
    const backendLog = `${GATEWAY_DIR}/logs/backend_current.log`;
    // 重命名旧日志文件
    try {
      if (existsSync(backendLog)) {
        const stats = statSync(backendLog);
        const oldTimestamp = new Date(stats.mtime).toISOString().replace(/[:.]/g, "-").slice(0, 19);
    ...
    runInPane(
      1,
      `mkdir -p ${GATEWAY_DIR}/logs && cd ${GATEWAY_DIR} && echo '后端日志: ${backendLog}' && npx tsx watch src/server/server.ts 2>&1 | tee -a '${backendLog}'`
    );
    await sleep(5000);

    const health = await this.checkBackendHealth();
    if (health) {
      log(`✅ 后端启动成功 (当前日志: backend_current.log)`, "green");
    } else {
      log(`⚠️ 后端可能未成功启动，请检查窗格 1 或日志: backend_current.log`, "yellow");
    }
    return health;
  },
  ...
  async checkFrontendHealth() {
    try {
      const result = exec('curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:5173', {
        timeout: 5000,
      });
  ...
  async checkBackendHealth() {
    try {
      const result = exec("curl -s http://127.0.0.1:3000/api/version", {
        timeout: 5000,
      });
  ...
  async getStatus() {
    const frontendRunning = (
      await exec('pgrep -f "vite --host 127.0.0.1" | head -1', {
        ignoreError: true,
      })
    ).trim();
    const backendRunning = (
      await exec('pgrep -f "tsx watch src/server/server.ts" | head -1', {
        ignoreError: true,
      })
    ).trim();
  ...
  async printStatus() {
    ...
    if (status.frontend.pid) {
      ...
      log(
        `  健康: ${status.frontend.healthy ? "✅" : "⚠️"}`,
        status.frontend.healthy ? "green" : "yellow"
      );
    } else {
      log("  状态: 未运行", "red");
    }
    ...
    if (status.backend.pid) {
      ...
      log(
        `  健康: ${status.backend.healthy ? "✅" : "⚠️"}`,
        status.backend.healthy ? "green" : "yellow"
      );
    } else {
      log("  状态: 未运行", "red");
    }
