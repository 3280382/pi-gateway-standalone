...
if ! command -v tmux &> /dev/null; then
    ...
    apt-get update && apt-get install -y tmux 2>/dev/null || brew install tmux 2>/dev/null || {
        echo "请手动Install tmux"
        exit 1
    }
fi

# Create session
echo -e "${BLUE}Create tmux 会话...${NC}"
...
echo ""
echo "操作说明:"
echo "  1. 你现在进入的是 AI 交互窗格（底部），Run着 pi"
...
echo ""
echo "🚪 Exit方式:"
echo "  Ctrl+b + d     分离（推荐，服务保持Run，可Resume）"
echo "  Ctrl+c         Stop当前窗格的服务"
echo "  exit           Exit当前窗格（pi/bash）"
echo "  关闭终端       强制关闭所有"
echo ""
