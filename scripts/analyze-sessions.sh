...
const stats = {
    ...
    totalToolCalls: 0,
    tools: {}, // 按工具名称统计
    bashCommands: {}, // bash 工具按具体Command统计
    failures: [], // Failed的调用详情
    errors: [], // 解析Error
};

// 常见的 bash Command列表（用于识别）
...
function processSessionFile(filePath) {
    ...
    try {
        ...
        stats.errors.push({ file: fileName, error: `读取Failed: ${err.message}` });
        return;
    }
    ...
    // 第二遍：处理 toolResult（判断Success/Failed）
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
...
// 主函数
function main() {
    console.log('📊 Start分析 session 文件...');
    ...
    console.log(`📁 详细报告已Save: ${OUTPUT_FILE}`);
...
console.log(`❌ Failed调用: ${result.failures.length}`);
console.log(`⚠️ 解析Error: ${result.errors.length}`);
NODE_SCRIPT
...
console.log(`   Failed次数: ${data.failures.length}`);
const overallSuccess = data.totalToolCalls > 0 
    ? ((1 - data.failures.length / data.totalToolCalls) * 100).toFixed(2)
    : '100.00';
console.log(`   整体Success率: ${overallSuccess}%`);

console.log('\n📋 各工具详情:');
console.log('─'.repeat(90));
console.log(`${pad('工具名', 18)} ${padStart('调用次数', 12)} ${padStart('Success', 10)} ${padStart('Failed', 10)} ${padStart('Success率', 12)}`);
console.log('─'.repeat(90));
...
console.log('\n' + '='.repeat(90));
console.log('🖥️  Bash Command统计（Top 30）');
console.log('='.repeat(90));
...
console.log('\n📋 Command详情:');
console.log('─'.repeat(90));
console.log(`${pad('Command', 18)} ${padStart('调用次数', 12)} ${padStart('Success', 10)} ${padStart('Failed', 10)} ${padStart('Success率', 12)}`);
console.log('─'.repeat(90));
...
if (data.failures.length === 0) {
    ...
    console.log(`\n共发现 ${data.failures.length} 次Failed调用，涉及 ${sortedFailures.length} 个工具:`);
    console.log('─'.repeat(90));
    
    for (const [tool, info] of sortedFailures) {
        console.log(`\n🔴 ${tool}: ${info.count} 次Failed`);
        if (info.examples.length > 0) {
            console.log('   FailedExample:');
            for (const example of info.examples) {
                console.log(`     - Error: ${example.error.substring(0, 80)}`);
...
for (const [name, stat] of tools) {
    ...
    if (rate < 95 && stat.calls > 10) {
        suggestions.push({
            ...
            tool: name,
            priority: '高',
            message: `${name} 工具Failed率 ${(100-rate).toFixed(1)}%，建议CheckArguments验证和Error处理`,
        });
    }
}

// 2. Bash CommandFailed
...
for (const [cmd, stat] of failedBash) {
    if (stat.failures > 2) {
        suggestions.push({
            ...
            tool: `bash:${cmd}`,
            priority: '中',
            message: `${cmd} CommandFailed ${stat.failures} 次，建议添加前置Check（如路径Exists性、权限等）`,
        });
    }
}
...
if (data.bashCommands['rm'] && data.bashCommands['rm'].calls > 20) {
    suggestions.push({
        ...
        tool: 'bash:rm',
        priority: '中',
        message: `rm Command使用 ${data.bashCommands['rm'].calls} 次，建议添加Confirm提示或备份机制`,
    });
}
...
if (data.bashCommands['sudo'] && data.bashCommands['sudo'].calls > 10) {
    suggestions.push({
        ...
        tool: 'bash:sudo',
        priority: '低',
        message: `sudo Command使用 ${data.bashCommands['sudo'].calls} 次，建议Check是否真的需要 root 权限`,
    });
}
...
if (data.bashCommands['cd'] && data.bashCommands['cd'].calls > 100) {
    suggestions.push({
        ...
        tool: 'bash:cd',
        priority: '低',
        message: `cd Command使用 ${data.bashCommands['cd'].calls} 次，考虑使用绝对路径减少目录Toggle`,
    });
}
...
if (data.totalToolCalls > 0) {
    console.log(`   Bash Command:   ${padStart(totalBash, 8)} (${((totalBash/data.totalToolCalls)*100).toFixed(1)}%)`);
...
echo "   - 分析Script: /root/pi-gateway-standalone/scripts/analyze-sessions.sh"
echo ""
